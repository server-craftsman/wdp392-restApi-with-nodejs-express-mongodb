...
export default class SampleRoute implements IRoute {
    ...
    private initializeRoutes() {
        ...
        this.router.get(`${API_PATH.SAMPLE}/search`, authMiddleWare([UserRoleEnum.LABORATORY_TECHNICIAN, UserRoleEnum.STAFF]), this.sampleController.searchSamples);

        // GET: domain:/api/sample/testing/ready -> Get samples ready for testing (RECEIVED status)
        this.router.get(`${API_PATH.SAMPLE}/testing/ready`, authMiddleWare([UserRoleEnum.LABORATORY_TECHNICIAN]), this.sampleController.getSamplesReadyForTesting);

        // GET: domain:/api/sample/testing/all -> Get all samples with TESTING status without pagination
        this.router.get(`${API_PATH.SAMPLE}/testing/all`, authMiddleWare([UserRoleEnum.LABORATORY_TECHNICIAN]), this.sampleController.getAllSamplesForTesting);
        ...
        this.router.get(`${API_PATH.SAMPLE}/:id`, authMiddleWare([UserRoleEnum.ADMIN, UserRoleEnum.MANAGER, UserRoleEnum.STAFF, UserRoleEnum.LABORATORY_TECHNICIAN, UserRoleEnum.CUSTOMER]), this.sampleController.getSampleById);

        // PUT: domain:/api/sample/:id/submit -> Submit a sample (by customer)
        this.router.put(`${API_PATH.SAMPLE}/:id/submit`, authMiddleWare([UserRoleEnum.CUSTOMER]), validationMiddleware(SubmitSampleDto), this.sampleController.submitSample);

        // PUT: domain:/api/sample/:id/receive -> Receive a sample (by staff)
        this.router.put(`${API_PATH.SAMPLE}/:id/receive`, authMiddleWare([UserRoleEnum.STAFF]), validationMiddleware(ReceiveSampleDto), this.sampleController.receiveSample);
        ...
        this.router.post(`${API_PATH.SAMPLE}/batch-submit`, authMiddleWare([UserRoleEnum.CUSTOMER]), validationMiddleware(BatchSubmitSamplesDto), this.sampleController.batchSubmitSamples);

        // POST: domain:/api/sample/batch-receive -> Batch receive multiple samples (by staff)
        this.router.post(`${API_PATH.SAMPLE}/batch-receive`, authMiddleWare([UserRoleEnum.STAFF]), validationMiddleware(BatchReceiveSamplesDto), this.sampleController.batchReceiveSamples);

        // POST: domain:/api/sample/upload-person-image -> Upload person image for a sample
        this.router.post(`${API_PATH.SAMPLE}/upload-person-image`, authMiddleWare(), uploadSingleFile('image'), this.sampleController.uploadPersonImage);

        // POST: domain:/api/samples/collect -> Collect sample at facility
        this.router.post(`${this.path}/collect`, authMiddleWare([UserRoleEnum.STAFF, UserRoleEnum.LABORATORY_TECHNICIAN]), validationMiddleware(CollectSampleDto), this.sampleController.collectSampleAtFacility);
    }
}
