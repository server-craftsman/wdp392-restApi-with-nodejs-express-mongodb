...
export default class AppointmentLogRoute implements IRoute {
    ...
    private initializeRoutes() {
        ...
        this.router.get(`${this.path}/appointment/:appointmentId`, authMiddleWare([UserRoleEnum.ADMIN, UserRoleEnum.MANAGER, UserRoleEnum.STAFF, UserRoleEnum.LABORATORY_TECHNICIAN]), this.appointmentLogController.getAppointmentLogs);
        ...
        this.router.get(`${this.path}/administrative-case/:caseId`, authMiddleWare([UserRoleEnum.ADMIN, UserRoleEnum.MANAGER, UserRoleEnum.STAFF]), this.appointmentLogController.getAdministrativeCaseLogs);

        // GET: domain:/api/appointment-log/action/:action -> Get logs by action type
        this.router.get(`${this.path}/action/:action`, authMiddleWare([UserRoleEnum.ADMIN, UserRoleEnum.MANAGER]), this.appointmentLogController.getLogsByAction);

        // GET: domain:/api/appointment-log/statistics -> Get log statistics for dashboard
        this.router.get(`${this.path}/statistics`, authMiddleWare([UserRoleEnum.ADMIN, UserRoleEnum.MANAGER]), this.appointmentLogController.getLogStatistics);
    }
}
